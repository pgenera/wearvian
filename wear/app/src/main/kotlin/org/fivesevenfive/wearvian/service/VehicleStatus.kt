 ...
 *                     the compact push does NOT populate them: 0.9.4-debug wired 0x01 to the UI and
 *                     it read "open" (bit 0) on a closed port — CONFIRMED on-vehicle 2026-07-05 that
 *                     charge-port state isn't in this push (it's in the poll/full-status message or
 *                     cloud). The sibling frunk/liftgate bits use the same sense and work, so this is
 *                     an unpopulated bit, not a sense bug. Don't decode 0x01/0x02 from the push.
